...
module.exports = {
  id: 'iprn_sms_v2',
  name: 'IPRN SMS V2 (iKangoo)',
  mode: 'manual',

  async listCountries() {
    return db.prepare(`
      SELECT DISTINCT country_code as code, COALESCE(country_name, country_code) as name
      FROM rates WHERE provider = 'iprn_sms_v2' AND country_code IS NOT NULL
    `).all();
  },
  ...
  async listOperators() {
    return db.prepare(`
      ...
      WHERE provider = 'iprn_sms_v2' AND operator IS NOT NULL
    `).all();
  },

  // Distinct ranges currently in pool (joined with iprn_sms_v2_range_meta to filter disabled)
  async listRanges() {
    return db.prepare(`
      SELECT
  ...
  async getNumber({ range, countryCode, operator } = {}) {
    let bot = null;
    try { bot = require('../workers/iprnSmsBotV2'); } catch (_) {}
    ...
    if (!row) {
      throw new Error(range
        ...
        : 'No fresh IPRN-SMS V2 numbers available — wait for next pool sync');
    }
    if (skipped > 0 || lost > 0) console.log(`[iprn_sms_v2-provider] assigned ${row.phone_number} (skipped=${skipped} stale, lost=${lost} race)`);
    return {
